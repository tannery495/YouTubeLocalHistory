...
function onStorageChanged(changes, area) {
    ...
    if (changes && changes.feedCache) {
        ...
        if (!subscriptionAutoRefreshInProgress) setStatus('', false);
        // Don't pop the feed grid over the analytics view if it's open.
        if (!subscriptionAutoRefreshInProgress) {
            if (channelActive && activeChannelInfo) renderChannelPage(activeChannelInfo);
            else if (!analyticsActive && !subscriptionsActive && !playlistsActive && !historyActive && !settingsActive && !watchLaterActive) render();
        }
    }
    if (changes && changes.durationCache) {
        durationCache = changes.durationCache.newValue || {};
        if (!subscriptionAutoRefreshInProgress && !analyticsActive && !subscriptionsActive && !playlistsActive && !historyActive && !settingsActive && !watchLaterActive && !channelActive) render();
    }
    if (changes && changes.shortsCache) {
        shortsCache = changes.shortsCache.newValue || {};
        if (!subscriptionAutoRefreshInProgress && !analyticsActive && !subscriptionsActive && !playlistsActive && !historyActive && !settingsActive && !watchLaterActive && !channelActive) render();
    }
    if (changes && Object.keys(changes).some((key) => key.startsWith('sub_'))) {
        ytStorage.getSubscriptionList().then((subscriptions) => {
            localSubscriptions = subscriptions || [];
            allVideos = (allVideos || []).filter((video) =>
                feedVideoMatchesAnySubscription(video, localSubscriptions)
            );
            if (subscriptionsActive) renderSubscriptions();
            else if (!analyticsActive && !playlistsActive && !historyActive && !settingsActive && !watchLaterActive && !channelActive) render();
        }).catch((error) => {
            console.warn('[subscriptions] could not sync feed after subscription change', error);
        });
    }
...
function init() {
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (analyticsActive || subscriptionsActive || playlistsActive || historyActive || watchLaterActive || settingsActive || channelActive) showFeed();
        else render();
        const query = searchInput.value.trim();
        // Refresh updates the local feed, not YouTube search results, so hide it
        // while the user is on a search view and restore it when search clears.
        setRefreshVisible(!query);
    ...
    const watchLaterBack = document.getElementById('watchLaterBack');
    if (watchLaterBack) watchLaterBack.addEventListener('click', () => {
        showPlaylists();
    });
    ...
    // Feed filters now live in Settings instead of the Refresh overflow menu.
    const unwatched = document.getElementById('unwatched');
    const hideMembers = document.getElementById('hideMembers');
    if (unwatched) {
        unwatched.checked = localStorage.getItem('ytvhtUnwatchedOnly') === 'true';
    }
    if (hideMembers) {
        hideMembers.checked = localStorage.getItem('ytvhtHideMembers') !== 'false';
    }

    // Close open video menus when clicking elsewhere.
    ...
    document.getElementById('feedSettingHideYouTubeRecommendations')?.addEventListener('change', () => {
        saveCurrentFeedSettings('YouTube recommendations');
    });
    initFeedDataSettings();
    unwatched?.addEventListener('change', () => {
        localStorage.setItem('ytvhtUnwatchedOnly', unwatched.checked ? 'true' : 'false');
        // Settings owns these controls too. Save the preference without
        // rendering the Home grid over the Settings view. The filter is
        // applied the next time the user opens Home/Subscriptions.
        if (!settingsActive) render();
    });
    hideMembers?.addEventListener('change', () => {
        localStorage.setItem('ytvhtHideMembers', hideMembers.checked ? 'true' : 'false');
        // Do not replace the Settings view with feed/channel content while a
        // preference is being changed.
        if (settingsActive) return;
        if (channelActive && typeof renderVisibleChannelItems === 'function') {
