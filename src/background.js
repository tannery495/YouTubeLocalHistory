...
function sendBrowseContinuationThroughTab(tabId, token, config) {
    return new Promise((resolve) => {
        chrome.tabs.sendMessage(
            tabId,
            { type: 'fetchYouTubeBrowseContinuationInTab', token, config },
            (response) => {
                if (chrome.runtime.lastError) {
                    resolve({ error: chrome.runtime.lastError.message });
                    return;
...
function sendChannelVideoScrapeThroughTab(tabId, limit) {
    return new Promise((resolve) => {
        chrome.tabs.sendMessage(
            tabId,
            { type: 'scrapeYouTubeChannelVideosInTab', limit },
            (response) => {
                if (chrome.runtime.lastError) {
...
async function fetchBrowseContinuationThroughYouTubeTab(token, config) {
    const tabs = await chrome.tabs.query({ url: ['*://www.youtube.com/*'] });
    let temporaryTab = null;
    let temporaryWindowId = null;
    let lastError = 'Could not contact a YouTube tab';
    ...
    async function tryTab(tab, attempts = 16) {
        if (!tab || tab.id == null) return null;
        for (let attempt = 0; attempt < attempts; attempt++) {
            const response = await sendBrowseContinuationThroughTab(tab.id, token, config);
            if (response.data) return response;
            lastError = response.error || lastError;
            await new Promise((resolve) => setTimeout(resolve, 250));
        }
...
async function scrapeChannelVideosThroughYouTubeTab(url, limit = 120) {
    const targetUrl = String(url || '').trim();
    if (!/^https:\/\/(?:www\.)?youtube\.com\//i.test(targetUrl)) {
        return { error: 'Invalid YouTube channel URL', items: [] };
    }

    let temporaryTab = null;
    let temporaryWindowId = null;
    let lastError = 'Could not scrape the YouTube channel page';

    try {
        const temporary = await createTemporaryYouTubeContext(targetUrl);
        temporaryTab = temporary.tab;
        temporaryWindowId = temporary.windowId;

        if (temporaryTab && temporaryTab.id != null) {
            await waitForTabReady(temporaryTab.id, 10000);
        }
        ...
        // The content script can arrive a moment after tab "complete".
        for (let attempt = 0; attempt < 12; attempt++) {
            const response = await sendChannelVideoScrapeThroughTab(temporaryTab.id, limit);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    (async () => {
        ...
        if (message.type === 'scrapeYouTubeChannelVideos') {
            const url = String(message.url || '').trim();
            const limit = Math.max(20, Math.min(200, Number(message.limit) || 120));
            try {
                sendResponse(await scrapeChannelVideosThroughYouTubeTab(url, limit));
            } catch (error) {
                sendResponse({
                    error: error && error.message ? error.message : String(error),
                    items: []
        ...
        if (message.type === 'fetchYouTubeBrowseContinuation') {
            const token = String(message.token || '');
            const config = message.config || {};
            if (!token || !config.clientVersion) {
            ...
            try {
                const tabResponse = await fetchBrowseContinuationThroughYouTubeTab(token, config);
                if (tabResponse.data) {
                    sendResponse(tabResponse);
                    return;
                }
                ...
                throw lastError || new Error('YouTube channel paging failed');
            } catch (error) {
                sendResponse({ error: error && error.message ? error.message : String(error) });
            }
            return;
...
// Ask the already-running YouTube content script for title/channel metadata.
// This avoids dynamic code injection and keeps the permission set smaller.
function requestWatchLaterVideoMeta(tabId, videoId) {
    return new Promise((resolve) => {
        if (tabId == null || !chrome.tabs || typeof chrome.tabs.sendMessage !== 'function') {
            resolve({ title: '', channelName: '' });
            return;
        }
        chrome.tabs.sendMessage(
            tabId,
            { type: 'extractWatchLaterVideoMeta', videoId },
            (response) => {
                if (chrome.runtime.lastError) {
                    resolve({ title: '', channelName: '' });
                    return;
                }
                resolve(response || { title: '', channelName: '' });
            }
        );
    });
}
...
async function handleAddWatchLater(info, tab) {
    ...
    // Best-effort title/channel from the existing content script; falls back to empty.
    let meta = { title: '', channelName: '' };
    try {
        if (tab && tab.id != null) {
            meta = await requestWatchLaterVideoMeta(tab.id, parsed.videoId);
        }
