...
function rendererSubscriberText(node) {
    if (!node || typeof node !== 'object') return '';
    const stack = [node];
    const seen = new Set();
    ...
    while (stack.length) {
        ...
        if (typeof value === 'string') {
            ...
            if (typeof normalizeSubscriberText === 'function') {
                const normalized = normalizeSubscriberText(clean);
...
function channelFromRenderer(cr) {
    ...
    return {
        ...
        thumbnail: avatar,
        subsText: (() => {
            // YouTube search channelRenderer currently puts the @handle in
            // subscriberCountText and the actual subscriber count in
            // videoCountText. Do not let the truthy handle short-circuit the
            // fallback search for a real subscriber count.
            const candidates = [
                runsText(cr.subscriberCountText),
                runsText(cr.videoCountText)
            ];
            for (const candidate of candidates) {
                ...
                if (typeof normalizeSubscriberText === 'function') {
                    const normalized = normalizeSubscriberText(clean);
                    if (normalized) return normalized;
                } else if (/\d/.test(clean) && /subscribers?/i.test(clean)) {
                    return clean;
                }
            }
            return rendererSubscriberText(cr);
        })(),
...
function buildChannelCard(ch) {
    ...
    subs.className = 'ch-subs';
    meta.appendChild(subs);

    const setSubscriberCount = (value, resolved = false) => {
        const compact = typeof compactSubscriberCountText === 'function'
            ? compactSubscriberCountText(value)
            : String(value || '').trim();

        subs.classList.remove('subscriber-loading', 'subscriber-empty');
        if (compact) {
            subs.textContent = compact;
        } else {
            subs.textContent = '';
            subs.classList.add(resolved ? 'subscriber-empty' : 'subscriber-loading');
        }
    };

    setSubscriberCount(ch.subsText, false);
    if ((!ch.subsText || !compactSubscriberCountText(ch.subsText)) &&
        typeof resolveChannelSubscriberText === 'function') {
        resolveChannelSubscriberText(ch).then((value) => {
            if (value) ch.subsText = value;
            setSubscriberCount(value, true);
        });
    }
...
function filteredYouTubeResults(query) {
    ...
    // Keep YouTube search consistent with Home and channel pages.
    // Preserve channel result cards, but hide members-only video results
    // whenever the global Hide member videos setting is enabled.
    const hideMemberVideos = document.getElementById('hideMembers')?.checked;
    if (hideMemberVideos) {
        results = results.filter((result) =>
            result._type === 'channel' || !videoIsMembersOnly(result)
        );
    }
