...
async function addVideoToLocalPlaylist(video) {
    ...
    return new Promise((resolve) => {
        ...
        records.forEach((playlist) => {
            ...
            const playlistItems = playlist._source === 'youtube'
                ? (playlist.localItems || {})
                : (playlist.items || {});
            const total = Object.keys(playlistItems).length;
            const alreadyAdded = !!(
                playlistItems[video.videoId] ||
                (playlist._source === 'youtube' && playlist.items && playlist.items[video.videoId])
            );
            count.textContent = alreadyAdded
                ? `Added · ${total} video${total === 1 ? '' : 's'}`
                : (playlist._source === 'youtube'
            ...
            if (alreadyAdded) {
                ...
                option.title = 'Already in this playlist';
                appendSvgMarkup(saveIcon, '<circle cx="12" cy="12" r="9"></circle><path d="m8 12 2.5 2.5L16 9"></path>');
            } else {
...
async function getVideoMenuSavedState(videoId) {
    if (!videoId) return { inWatchLater: false, inPlaylist: false };

    let inWatchLater = false;
    let inPlaylist = false;

    try {
        inWatchLater = !!(await ytStorage.getWatchLater(videoId));
    } catch (_) { /* keep default */ }

    try {
        const stored = await chrome.storage.local.get(['localVideoPlaylists']);
        const localPlaylists = stored.localVideoPlaylists || {};
        inPlaylist = Object.values(localPlaylists).some((playlist) =>
            !!(playlist && playlist.items && playlist.items[videoId])
        );
    } catch (_) { /* keep default */ }

    if (!inPlaylist) {
        try {
            const savedPlaylists = await ytStorage.getAllPlaylists();
            inPlaylist = Object.values(savedPlaylists || {}).some((playlist) => !!(
                playlist && (
                    (playlist.localItems && playlist.localItems[videoId]) ||
                    (playlist.items && playlist.items[videoId])
                )
            ));
        } catch (_) { /* keep default */ }
    }

    return { inWatchLater, inPlaylist };
}
...
function buildVideoMenu(video, options) {
    ...
    const actions = [];
    let watchLaterMenuText = null;
    let playlistMenuText = null;

    async function refreshSavedStateLabels() {
        if (!watchLaterMenuText && !playlistMenuText) return;
        const state = await getVideoMenuSavedState(video.videoId);
        if (watchLaterMenuText) {
            watchLaterMenuText.textContent = state.inWatchLater
                ? tFeed('feed_saved_to_watch_later', 'Saved to Watch Later')
                : tFeed('feed_save_to_watch_later', 'Save to Watch Later');
        }
        if (playlistMenuText) {
            playlistMenuText.textContent = state.inPlaylist
                ? tFeed('feed_manage_playlists', 'Manage playlists')
                : tFeed('feed_add_to_playlist', 'Add to playlist');
        }
    }
    ...
    if (options.showUnsubscribe) {
        ...
        [tFeed('feed_unsubscribe', 'Unsubscribe'), '<circle cx="9" cy="8" r="3"></circle><path d="M3.5 18c.5-3.5 2.4-5 5.5-5 1.2 0 2.2.2 3 .7"></path><path d="M15 11h6"></path>', async () => {
            ...
            await removeSubscriptionAndPruneFeed(subscription);
    ...
    if (options.showWatchLater !== false) {
        actions.push(
    ...
    actions.forEach(([label, iconPaths, action]) => {
        ...
        if (label === tFeed('feed_save_to_watch_later', 'Save to Watch Later')) watchLaterMenuText = text;
        if (label === tFeed('feed_add_to_playlist', 'Add to playlist')) playlistMenuText = text;
        ...
        item.addEventListener('click', async (event) => {
            ...
            try {
                ...
                if (confirmation) {
                    ...
                    setTimeout(() => { refreshSavedStateLabels().catch(() => {}); }, 1200);
                } else {
                    refreshSavedStateLabels().catch(() => {});
                }
            } catch (error) {
    ...
    toggle.addEventListener('click', (event) => {
        ...
        if (opening) refreshSavedStateLabels().catch(() => {});
...
function buildCard(video, options) {
    options = options || {};
...
function videoViewsText(video) {
    // Channel/search renderers can expose a ready-made string even when the
    // abbreviated value does not parse cleanly to a number.
    if (video && video._viewsText) return video._viewsText;
    return formatViews(video && video.views);
}

function videoAgeText(video) {
    // Prefer the exact relative time YouTube supplied when available.
    if (video && video._whenText) return video._whenText;
    if (video && video.published) return relativeTime(video.published);
    return '';
}
