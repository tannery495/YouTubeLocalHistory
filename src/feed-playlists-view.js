...
function appendPlaylistDetailActions(record, container, onDeleted, onRenamed) {
    ...
    rename.className = 'btn playlist-detail-inline-action';
    rename.type = 'button';
    rename.innerHTML = '<svg class="playlist-detail-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg><span>Rename</span>';
    ...
    const remove = document.createElement('button');
    remove.className = 'btn danger playlist-detail-inline-action';
    remove.type = 'button';
    remove.innerHTML = '<svg class="playlist-detail-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"></path><path d="m9 7 .7-2h4.6l.7 2"></path><path d="M7 7l1 13h8l1-13"></path></svg><span>Remove</span>';
    remove.addEventListener('click', async () => {
        ...
        try {
            ...
            setStatus('Could not remove playlist.', true);
        }
    });
...
function makeWatchLaterPlaylistFallback() {
    ...
    icon.style.strokeWidth = '1.6';
    icon.style.strokeLinecap = 'round';
    icon.style.strokeLinejoin = 'round';
    appendPlaylistSvgMarkup(icon, '<circle cx="12" cy="12" r="8.5"></circle><path d="M12 7.5v5l3.2 2"></path>');
    fallback.appendChild(icon);
    return fallback;
}

function appendWatchLaterPlaylistCard(list, watchLaterItems) {
    const records = Object.values(watchLaterItems || {})
        .filter((item) => item && item.videoId)
        .sort((a, b) => Number(b.addedAt || b.savedAt || 0) - Number(a.addedAt || a.savedAt || 0));

    const row = document.createElement('div');
    row.className = 'playlist-row playlist-row-watch-later';

    const openWatchLater = (event) => {
        event.preventDefault();
        showWatchLater();
    };
    ...
    thumbLink.href = '#';
    thumbLink.addEventListener('click', openWatchLater);

    const thumb = document.createElement('div');
    thumb.className = 'playlist-thumb';
    const newest = records[0];
    const thumbnailUrl = newest && (
        newest.thumbnail ||
        (newest.videoId ? `https://i.ytimg.com/vi/${newest.videoId}/hqdefault.jpg` : '')
    );
    if (thumbnailUrl) {
        ...
        image.addEventListener('error', () => {
            ...
            if (!thumb.querySelector('.playlist-thumb-fallback')) {
                thumb.prepend(makeWatchLaterPlaylistFallback());
            }
        }, { once: true });
        thumb.appendChild(image);
    } else {
        thumb.appendChild(makeWatchLaterPlaylistFallback());
    }
    ...
    name.className = 'playlist-name watch-later-playlist-name';
    name.href = '#';
    const clockIcon = document.createElement('span');
    clockIcon.className = 'watch-later-title-icon';
    clockIcon.setAttribute('aria-hidden', 'true');
    clockIcon.innerHTML = '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="8.5"></circle><path d="M12 7.5v5l3.2 2"></path></svg>';
    const label = document.createElement('span');
    label.textContent = feedMessage('tab_watchlater') || 'Watch Later';
    name.append(clockIcon, label);
    name.addEventListener('click', openWatchLater);
    titleRow.appendChild(name);

    text.appendChild(titleRow);
    const meta = document.createElement('span');
    meta.className = 'playlist-meta';
    meta.textContent = `${records.length} video${records.length === 1 ? '' : 's'}`;
...
async function renderPlaylists() {
    ...
    try {
        watchLaterItems = await ytStorage.getAllWatchLater();
    } catch (_) { /* show the built-in Watch Later card empty */ }
    ...
    const totalPlaylistCount = records.length + 1; // Watch Later is the built-in local playlist.
    count.textContent = String(totalPlaylistCount);
    const playlistCountLabel = `${totalPlaylistCount} playlist${totalPlaylistCount === 1 ? '' : 's'}`;
    count.title = playlistCountLabel;
    count.setAttribute('aria-label', playlistCountLabel);
    empty.style.display = 'none';

    appendWatchLaterPlaylistCard(list, watchLaterItems);
    ...
    records.forEach((record) => {
        ...
        text.appendChild(titleRow);
        const metaText = playlistMetaText(record);
        if (metaText) {
            const meta = document.createElement('span');
            meta.className = 'playlist-meta';
...
function setPlaylistsNavBackMode() {
    // Keep the sidebar navigation stable while viewing a playlist detail.
    // The in-page Back button handles returning to the playlists grid.
    const nav = document.getElementById('navPlaylists');
    const label = nav && nav.querySelector('span[data-i18n="feed_nav_playlists"]');
    if (label) label.textContent = feedMessage('feed_nav_playlists') || 'Playlists';
}

function playlistDetailUpdatedText(record) {
    const timestamp = Number(record.lastUpdated || record.timestamp || 0);
    if (!timestamp) return '';
    ...
    return `Updated ${new Date(timestamp).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}`;
}

function buildPlaylistDetailCover(record) {
    const cover = document.createElement('div');
    cover.className = 'local-playlist-detail-cover';
    const thumbnailUrl = playlistCoverThumbnail(record);
    if (thumbnailUrl) {
        const image = document.createElement('img');
        image.src = thumbnailUrl;
    ...
    fallback.className = 'local-playlist-detail-cover-icon';
...
function addPlaylistDetailBackButton(detail) {
...
function buildPlaylistDetailLoading(record) {
    ...
    detail.className = 'local-playlist-detail local-playlist-detail--simple playlist-detail-loading';

    const header = document.createElement('div');
    header.className = 'history-toolbar local-playlist-detail-header local-playlist-detail-header--compact';
    const titleWrap = document.createElement('div');
    titleWrap.className = 'local-playlist-detail-title-wrap';
    const heading = document.createElement('h2');
    heading.className = 'history-title local-playlist-title';
    heading.textContent = decodeHtmlEntities(record.title || 'Playlist');
    const meta = document.createElement('div');
    meta.className = 'history-count local-playlist-count';
    ...
    for (let i = 0; i < 3; i++) {
        ...
        rowText.className = 'playlist-detail-skeleton-text';
        ['title', 'meta', 'channel'].forEach((kind) => {
            const line = document.createElement('div');
            line.className = `playlist-detail-skeleton-line ${kind}`;
            rowText.appendChild(line);
        });
        row.append(thumb, rowText);
...
async function renderLocalPlaylistDetail(record, allowImport = true) {
    ...
    list.textContent = '';
    const detail = document.createElement('div');
    detail.className = 'local-playlist-detail local-playlist-detail--simple';
    ...
    const meta = document.createElement('div');
    meta.className = 'history-count local-playlist-count';
    meta.textContent = `${videos.length} video${videos.length === 1 ? '' : 's'}`;
    ...
    toolbarActions.className = 'local-playlist-toolbar-actions local-playlist-toolbar-actions--inline';
    if (record._local) {
        const addVideos = document.createElement('button');
        addVideos.className = 'btn primary playlist-add-videos';
        ...
        addVideos.addEventListener('click', () => {
            ...
            showFeed();
            setStatus('Use the ⋮ menu on a video and choose Add to local playlist.', false);
        });
        toolbarActions.appendChild(addVideos);
    }
    ...
    if (!record._local && record.playlistId) {
        ...
        toolbarActions.insertBefore(openYouTube, toolbarActions.firstChild);
    }
...
function showPlaylists() {
    ...
    setPageCountVisible('playlistsCount');
