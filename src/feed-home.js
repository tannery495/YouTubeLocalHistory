...
function currentView() {
    ...
    if (!q) {
        ...
        if (!shortsOnly && !subscriptionsChronological) {
            // Prefer fully enriched cards, but never turn a valid feed into an
            // empty Home page just because YouTube duration metadata is still
            // loading or temporarily unavailable.
            const enriched = list.filter(hasHomeDurationBadge);
            if (enriched.length) list = enriched;
        ...
        if (subscriptionsChronological) {
            // Same graceful fallback for Subscriptions: RSS/channel-page videos
            // remain visible even when a duration lookup failed.
            const enriched = list.filter(hasSubscriptionDurationBadge);
            if (enriched.length) list = enriched;
        }
...
function render() {
    ...
    // Clear centered empty-state styling before rendering.
    empty.classList.remove('home-empty-card', 'centered-empty-card');
    ...
    // No plain "Your feed" label; only show a heading for search context.
    const headingText = q ? `Search results for “${q}”` : '';
    ...
    if (allVideos.length === 0 && !q) {
        ...
        document.body.classList.add('empty-page-no-scroll');
        empty.classList.add(!shortsOnly && !subscriptionsChronological ? 'home-empty-card' : 'centered-empty-card');
        const emptyTitle = shortsOnly ? 'No Shorts yet' : (subscriptionsChronological ? 'No subscription videos yet' : 'No videos yet');
        const emptyMessage = shortsOnly
            ? 'Shorts from your subscribed channels will appear here.'
            : (subscriptionsChronological
                ? 'Subscribe to channels to start building your feed.'
                : 'Subscribe to channels, then refresh your feed.');
        const emptyIcon = shortsOnly
        ...
        empty.innerHTML = '<div class="' + (!shortsOnly && !subscriptionsChronological ? 'home-empty-panel' : 'empty-panel') + '">' +
    ...
    if (list.length === 0) {
        ...
        empty.style.display = 'flex';
        document.body.classList.add('empty-page-no-scroll');
        empty.classList.add('centered-empty-card');

        let emptyTitle = 'No videos match';
        let emptyMessage = 'Try refreshing your feed or adjusting your filters.';
        let emptyIcon = '<svg viewBox="0 0 24 24"><path d="M3 10.8 12 3l9 7.8"></path><path d="M5.5 9.5V21h13V9.5"></path><path d="M9.5 21v-7h5v7"></path></svg>';

        if (shortsOnly) {
            emptyTitle = 'No Shorts yet';
            emptyMessage = 'No Shorts from your subscribed channels are available right now.';
            emptyIcon = '<svg viewBox="0 0 24 24"><path d="m13.5 2-7 11h5l-1 9 7-12h-5l1-8Z"></path></svg>';
        } else if (subscriptionsChronological) {
            emptyTitle = 'No subscription videos yet';
            emptyMessage = 'Refresh to check for new videos from your subscribed channels.';
            emptyIcon = '<svg viewBox="0 0 24 24"><rect x="3" y="6" width="18" height="14" rx="2"></rect><path d="m9 3 3 3 3-3"></path><path d="m10 10 5 3-5 3v-6Z"></path></svg>';
        }

        empty.innerHTML = '<div class="empty-panel">' +
            '<span class="empty-state-icon" aria-hidden="true">' + emptyIcon + '</span>' +
            '<strong>' + emptyTitle + '</strong>' +
            '<span>' + emptyMessage + '</span>' +
            '</div>';
