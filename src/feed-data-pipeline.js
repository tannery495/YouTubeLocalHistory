...
// Privacy: Local Feed does not read, write, or enumerate browser cookies.
// Requests to YouTube may still use the browser's existing YouTube session via
// fetch(..., { credentials: 'include' }), which sends cookies directly to
// YouTube without exposing their values to the extension.
...
async function refreshFeedNow(force = false) {
    ...
    try {
        ...
        let previousVideos = [];
        if (!force) {
            // Keep both the persisted cache and the current in-memory feed. The
            // latter matters when a user subscribes from a channel page: those
            // channel-tab cards already contain usable title/date/duration
            // metadata, but may not have been written to feedCache yet.
            const memoryVideos = Array.isArray(allVideos)
                ? allVideos.filter((video) => feedVideoMatchesAnySubscription(video, subs))
                : [];
            let cachedVideos = [];
            try {
                const cache = (await ytStorage.getFeedCache()) || {};
                cachedVideos = Array.isArray(cache.videos)
                    ? cache.videos.filter((video) => feedVideoMatchesAnySubscription(video, subs))
                    : [];
            } catch (_) { /* fast refresh can still continue without old cache */ }

            const previousById = new Map();
            memoryVideos.concat(cachedVideos).forEach((video) => {
                if (!video || !video.videoId) return;
                const existing = previousById.get(video.videoId);
                previousById.set(video.videoId, existing ? { ...video, ...existing } : video);
            });
            previousVideos = Array.from(previousById.values());
        }
        const { videos, diagnostics } = await fetchAllFeeds(subs, force);
        if (!shouldContinue()) return;

        // RSS is the authoritative fresh source, but it intentionally lacks
        // duration/Shorts metadata. Merge any already-known metadata from the
        // channel page/cache instead of discarding it when video IDs overlap.
        ...
        videos.forEach((video) => {
            if (!video || !video.videoId) return;
            const previous = previousById.get(video.videoId) || {};
            const merged = { ...previous, ...video };
            // Preserve useful YouTube-page metadata when RSS has no equivalent.
            if (!video._durationText && previous._durationText) merged._durationText = previous._durationText;
            if (!video.duration && previous.duration) merged.duration = previous.duration;
            if (video.isShort === undefined && previous.isShort !== undefined) merged.isShort = previous.isShort;
            if (!video._whenText && previous._whenText) merged._whenText = previous._whenText;
            if (!video.views && previous.views) merged.views = previous.views;
            if (!video.channelThumbnail && previous.channelThumbnail) merged.channelThumbnail = previous.channelThumbnail;
            if (!video.channelUrl && previous.channelUrl) merged.channelUrl = previous.channelUrl;
            uniqueById.set(video.videoId, merged);
        });
        previousVideos.forEach((video) => {
            if (!video || !video.videoId || uniqueById.has(video.videoId)) return;
            uniqueById.set(video.videoId, video);
        });
        const trimmed = feedCore.selectFeedVideos(Array.from(uniqueById.values()), TOTAL_FEED_LIMIT);
