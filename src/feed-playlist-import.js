...
async function enrichLocalPlaylistMetadata(record, videos) {
    ...
    videos.forEach((video) => {
        ...
        const missingIdentity = !video.title || video.title === 'YouTube video' || !video.channelName;
        if ((!video.published || !video.duration || missingIdentity) && video.videoId) missing.push(video);
    });
    if (!missing.length) return;

    await ensureConsentCookie();
    await runPool(missing.slice(0, 80), 6, async (video) => {
        const missingIdentity = !video.title || video.title === 'YouTube video' || !video.channelName;
        const [metadata, identityPage] = await Promise.all([
            fetchSearchMetadata(video.videoId),
            missingIdentity && typeof searchYouTube === 'function'
                ? searchYouTube(video.videoId).catch(() => null)
        ...
        if (identityPage && Array.isArray(identityPage.results)) {
            const exact = identityPage.results.find((item) =>
                item && item._type !== 'channel' && item.videoId === video.videoId
            );
            if (exact) {
                if (exact.title) video.title = exact.title;
                if (exact.channelName) video.channelName = exact.channelName;
                if (exact.channelUrl) video.channelUrl = exact.channelUrl;
                if (exact.channelThumbnail) video.channelThumbnail = exact.channelThumbnail;
                if (exact.thumbnail) video.thumbnail = exact.thumbnail;
                if (exact._viewsText) video._viewsText = exact._viewsText;
                if (exact._whenText) video._whenText = exact._whenText;
                if (exact._durationText) video._durationText = exact._durationText;
            }
        }
    });
