// Dedicated feed page for YouTube Local Feed.
//
...
let subscriptionAutoRefreshInProgress = false;
...
function normalizedFeedChannelIdentity(value) {
...
function feedVideoMatchesSubscription(video, subscription) {
    if (!video || !subscription) return false;

    const videoName = String(video.channelName || '').trim().toLowerCase();
    const subName = String(subscription.channelName || '').trim().toLowerCase();
    if (videoName && subName && videoName === subName) return true;
    ...
    const videoIds = new Set([
        ...
        video.handle,
        video.channelUrl
    ].map(normalizedFeedChannelIdentity).filter(Boolean));
    ...
    return [
        ...
        .some((id) => videoIds.has(id));
}

function feedVideoMatchesAnySubscription(video, subscriptions = localSubscriptions) {
    return Array.isArray(subscriptions) && subscriptions.some((sub) =>
        feedVideoMatchesSubscription(video, sub)
    );
}

async function removeSubscriptionAndPruneFeed(subscription) {
    ...
    try {
        ...
        localSubscriptions = (localSubscriptions || []).filter((sub) => sub.id !== subscription.id);
    }

    allVideos = (allVideos || []).filter((video) =>
        !feedVideoMatchesSubscription(video, subscription)
    );

    try {
        const cache = (await ytStorage.getFeedCache()) || {};
        const cachedVideos = Array.isArray(cache.videos) ? cache.videos : [];
        const filteredVideos = cachedVideos.filter((video) =>
            !feedVideoMatchesSubscription(video, subscription)
        );
        if (filteredVideos.length !== cachedVideos.length) {
            await ytStorage.setFeedCache({
                ...cache,
                updatedAt: Date.now(),
        ...
        console.warn('[subscriptions] could not prune feed cache after unsubscribe', error);
    }
}
